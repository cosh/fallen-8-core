...
| `F8_AGENT_MODEL`                                           | `phi4-mini:latest`               | The model the sidecar pulls for the agent purpose, pulled only when `F8_AGENTS=true`. It is the INSTANCE that resolves the model, so this only has to name the same one `Fallen8__Chat__Ollama__Models__Agent` does |
