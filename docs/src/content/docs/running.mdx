...
Image tags mirror the git release tags; `latest` always points at the newest release. The one
difference to `env:up`: the GPU overlay is skipped, because the NLP transformer tier only
exists as a local build (the published NLP image is the CPU tier, linux/amd64). And for just
Fallen-8 itself (engine + REST API + F8 Studio in one container), no sidecars:
...
`npm run env:up` auto-detects an NVIDIA GPU (`nvidia-smi` present) and hands it to the two
containers that can use one: Ollama runs assist inference accelerated, and the NLP sidecar swaps to
the `en_core_web_trf` transformer model on the device instead of `en_core_web_lg` on the CPU.
Otherwise everything runs CPU-only. That model is baked in at build time, so flipping `F8_GPU`
rebuilds the `nlp` image (transformer weights plus the CUDA extras): the first `env:up` after the
switch is slow. Force it with `F8_GPU`:
