...
[observability](/observability/) stack. Every feature is on except the
[agent host](/agents/), which is off by default at both ends, and nothing asks for
...
GHCR (`ghcr.io/cosh/fallen-8-core` plus `-api`, `-studio`, `-mcp`, `-integrations`, `-agents`,
`-ollama`, `-nlp`), so the same environment runs without building anything:
...
| `F8_AGENTS`                                                | `false`                          | The [agent host](/agents/), on its own profile and with no host port. The one capability here that is OFF by default, at both ends: this brings up the sidecar AND opens the instance's `/agents/*` routes, and an agent can do whatever the MCP tiers allow |
| `F8_AGENT_MODEL`                                           | `phi4-mini:latest`               | The model the sidecar pulls for the agent purpose, pulled only when `F8_AGENTS=true` and `F8_PULL_ASSIST` is not `0`. The second condition is what keeps a hosted-chat overlay from downloading a model nothing there will ask for, since the instance answers the agent purpose at its provider; set `F8_PULL_ASSIST=1` to pull it locally anyway. It is the INSTANCE that resolves the model, so this only has to name the same one `Fallen8__Chat__Ollama__Models__Agent` does |
