...
One command brings up the whole thing: engine, REST API, [F8 Studio](/fallen-8-core/studio/), the model
sidecar behind [semantic traversal](/fallen-8-core/semantic-traversal/) and Studio's natural-language
assist, the document and NLP sidecars behind the [semantic layer](/fallen-8-core/unstructured-ingestion/),
the [MCP server](/fallen-8-core/mcp-server/) that AI agents connect to, and the full
[observability](/fallen-8-core/observability/) stack. Every feature is on and nothing asks for
...
| MCP server                    | http://localhost:8090                 | The [AI-agent surface](/fallen-8-core/mcp-server/), anonymous and read-only in this environment (`F8_MCP_PORT`)                     |
| Document conversion (docling) | http://localhost:5001                 | Converts PDF, Office and HTML uploads for [ingestion](/fallen-8-core/unstructured-ingestion/) (`F8_DOCLING_PORT`)                   |
| NLP enrichment (spaCy)        | http://localhost:8100                 | Named entities and key terms per chunk, folded into the entity graph (`F8_NLP_PORT`)                                                |
| Observability (Grafana)       | http://localhost:3000                 | The single pane for [metrics, traces and logs](/fallen-8-core/observability/) (`F8_GRAFANA_PORT`)                                   |
...
harden the MCP server separately (see [Security](/fallen-8-core/security/)).
...
`npm run env:logs`. The docling image is a separate ~4.4 GB pull on that same first start
...
| `F8_API_KEY`                                               | _(unset)_                        | Set it and the **entire data plane** requires the key; unset = open. The MCP bridge receives it for its own downstream calls but keeps its own caller auth (`F8_MCP_AUTH_MODE`) ([Security](/fallen-8-core/security/)) |
...
| `F8_INGESTION`                                             | `true`                           | [Document ingestion](/fallen-8-core/unstructured-ingestion/). `false` skips the docling sidecar (a ~4.4 GB image) **and** answers 403 on the document endpoints, including txt/md |
...
| `F8_INTEGRATIONS`                                          | `true`                           | The [integrations](/fallen-8-core/integrations/) runtime, on its own profile and with no host port. `false` skips the sidecar and the four `/integrations/*` routes then refuse the capability |
...
| `F8_MCP_AUTH_MODE`                                         | `None`                           | Caller auth for the [MCP server](/fallen-8-core/mcp-server/). Anything reachable off-box wants `StaticToken` plus `F8_MCP_TOKEN`                           |
...
| `F8_TENANT_ID` / `F8_INSTANCE_ID`                          | `local` / `f8-local`             | This instance's [fleet identity](/fallen-8-core/observability/) in the dashboards (`F8_TENANT_NAME` and `F8_INSTANCE_NAME` set the display names)           |
...
([how to run it](/fallen-8-core/mcp-server/)).
...
| `Fallen8:Metadata:Directory`                  | `<app base>/metadata`                   | [Save games](/fallen-8-core/save-games/)                   |
| `Fallen8:Namespaces:MaxNamespaces`            | `10000`                                 | [Namespaces](/fallen-8-core/namespaces/)                   |
| `Fallen8:Namespaces:LoadOnStartup`            | `true` (per-namespace overridable)      | [Namespaces](/fallen-8-core/namespaces/#startup-load)      |
| `Fallen8:Namespaces:StartupLoadMode`          | `Catalog` (`All`, `DefaultOnly`)        | [Namespaces](/fallen-8-core/namespaces/#startup-load)      |
...
| `Fallen8:Security:AllowedCorsOrigins`         | `[]` (deny all cross-origin)            | [Standalone F8 Studio](/fallen-8-core/standalone-ui/)      |
| `Fallen8:Security:BenchmarkMaxIterations`     | `10000`                                 | [Benchmark](/fallen-8-core/benchmark/)                     |
| `Fallen8:Embedding:Enabled`                   | `false` (bare run)                      | [Semantic traversal](/fallen-8-core/semantic-traversal/)   |
| `Fallen8:Chat:Enabled`                        | `false` (bare run)                      | [F8 Studio](/fallen-8-core/studio/)                        |
| `Fallen8:Ingestion:Enabled`                   | `false` (bare run)                      | [Semantic layer](/fallen-8-core/unstructured-ingestion/)   |
| `Fallen8:Nlp:Enabled`                         | `false` (bare run)                      | [Semantic layer](/fallen-8-core/unstructured-ingestion/)   |
| `Fallen8:ChangeFeed:Enabled`                  | `true`                                  | [Change feed](/fallen-8-core/change-feed/)                 |
| `Fallen8:BulkIO:ImportBatchSize`              | `10000`                                 | [Bulk import/export](/fallen-8-core/bulk-import-export/)   |
| `Fallen8:Analytics:DefaultTimeBudgetSeconds`  | `30` (max `300`)                        | [Graph analytics](/fallen-8-core/graph-analytics/)         |
| `Fallen8:Observability:Prometheus:Enabled`    | `false`                                 | [Observability](/fallen-8-core/observability/)             |
| `Fallen8:Observability:Otlp:Endpoint`         | `null` (no push)                        | [Observability](/fallen-8-core/observability/)             |
...
- [Architecture](/fallen-8-core/architecture/): how the containers, engine and sidecars fit together
- [Security](/fallen-8-core/security/): the API key you set at launch
- [Studio](/fallen-8-core/studio/): the browser UI this serves
- [MCP server](/fallen-8-core/mcp-server/): the agent surface on `:8090` and how to secure it
- [Observability](/fallen-8-core/observability/): the Grafana pane and the fleet identity knobs
