...
| ---------------------- | ---------------------- | ------------------------------------------------------------------ |
| F8 Studio (UI)         | http://localhost:8081  | The browser UI, its own container (override with `F8_UI_PORT`)      |
| F8 REST API            | http://localhost:8080  | The engine + REST surface, the data plane                           |
...
| `F8_UI_PORT`       | `8081`                           | Host port for the F8 Studio UI container (`env:up` runs the UI decoupled by default); the data plane allow-lists this origin |
...
runs with a bare `docker compose up` (no overlay). The runtime `config.js` seam, the image, and the
CORS rules are in [Standalone F8 Studio](/fallen-8-core/standalone-ui/).
