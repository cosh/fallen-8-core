...
| Model sidecar (Ollama)        | http://localhost:11434                | Serves `bge-m3` for embeddings and the assist models for NL assist. Replaceable by [Nahil](/nahil/)          |
...
The pre-seed covers the same set the container would pull: `phi4-mini`, `phi4-f8-mini`, `phi4-f8`
unless `F8_PULL_PHI4F8=0`, and `bge-m3` unless `F8_EMBEDDINGS=false`. So an environment seeded here
and started offline has a model behind every capability it advertises.
...
set once and keep - credentials included; [Nahil](/nahil/)'s `F8_NAHIL_API_KEY` is the canonical
case. To start one, copy
