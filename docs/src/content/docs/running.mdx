...
The log line names the build being served, and that id is the leading half of the sha256 of the
registry manifest, so you can compare it with what is published:

```bash
docker compose exec ollama ollama list | grep '^phi4-f8-mini:'
curl -s https://registry.ollama.ai/v2/stoic_hellman_728/phi4-f8-mini/manifests/latest | sha256sum | cut -c1-12
```

To pin a build instead of tracking `:latest`, give the repo variables a version tag -
`F8_DELEGATE_REPO=stoic_hellman_728/phi4-f8-mini:v0.0.34`. Those tags exist from the release that
created them onward: tagging the repository `vX.Y.Z` also tags the published models. The docling image is a separate ~4.4 GB pull on that same first start
