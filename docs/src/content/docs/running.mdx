...
| `F8_INTEGRATIONS`                                          | `true`                           | The [integrations](/integrations/) runtime, on its own profile and with no host port. `false` skips the sidecar and the four `/integrations/*` routes then refuse the capability |
