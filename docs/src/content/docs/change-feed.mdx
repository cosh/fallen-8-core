...
In F8 Studio the feed is visible as the [Events panel](/fallen-8-core/studio/#events): a bell in the top bar that counts filtered events live, opening onto the newest 100 with the same filter grammar.
...
| Auth | The same fallback policy as every route: no credential is required until `Fallen8:Security:ApiKey` is set, after which every request needs one. Nothing is loopback-only, so an unauthenticated instance is open to anyone who can reach the port (see [security](/fallen-8-core/security/)) |
| `200` | The SSE stream |
...
Each namespace owns an independent feed: its own epoch, catch-up buffer, and subscriber limit; see [namespaces](/fallen-8-core/namespaces/). Filters are plain query parameters, never compiled code (see [security](/fallen-8-core/security/)).
...
One JSON object per `data:` line. Fields are **absent** (not null) when they do not apply to the kind. Payloads never contain property values, re-fetch the element when you need a value (see [graph model](/fallen-8-core/graph-model/)).
...
| `edgePropertyId` | string | `edgeCreated`: the edge's type ([edge type vs label](/fallen-8-core/graph-model/#edge-type-vs-label)) |
...
`$embeddingModel:<name>` (see [graph model](/fallen-8-core/graph-model/)). Removing an embedding is a
...
| `delegateWrite` | A committed delegate (plugin write) transaction, whose effect the feed cannot express as element deltas. Emitted for every such commit whatever the body touched, so an [analytics](/fallen-8-core/graph-analytics/) run with a write-back property costs one per 50 000-vertex chunk | no |
...
- [Graph model](/fallen-8-core/graph-model/): the elements, properties, and transactions whose commits the feed reports
- [Namespaces](/fallen-8-core/namespaces/): the per-namespace feed and `/ns/{ns}/…` routing
- [Security](/fallen-8-core/security/): the API key and why the feed needs no dynamic code
- [REST API](/fallen-8-core/rest-api/): REST conventions, problem+json, and the OpenAPI document
