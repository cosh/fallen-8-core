...
One limit does remain: plugin lookup **by name** —
...
discovery and works there. The same gap means `IndexFactory.TryCreateIndex`, whose only way in is a plugin
name, cannot create an index in a browser at all — so no index scans and no vector search yet.
