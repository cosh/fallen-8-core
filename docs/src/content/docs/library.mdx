...
to it over [REST](/rest-api/). The second is to reference the engine as a .NET library and
...
[graph model](/graph-model/), [path finding](/path-finding/),
[subgraphs](/subgraphs/), [indexes](/indexes/),
[analytics](/graph-analytics/) and [save games](/save-games/) is reachable
...
| Persistence: checkpoints, the write-ahead log, save games                            | The API key and every other perimeter control ([security](/security/))   |
...
| One graph per `Fallen8` instance                                                     | [Namespaces](/namespaces/), which are an apiApp-level collection          |
...
[edge type vs label](/graph-model/#edge-type-vs-label).
...
| `new Fallen8(loggerFactory, changeFeedOptions)`                        | The [change feed](/change-feed/) as an in-order event stream     |
| `new Fallen8(loggerFactory, writeAheadLogOptions, ...)`                | A [write-ahead log](/save-games/) plus the optional compilers    |
...
- [Graph model](/graph-model/): the vertex, edge, property and label semantics the API exposes
- [Capacity and performance](/capacity-and-performance/): what a graph costs in memory and how fast writes go
- [Plugin registration](/plugin-registration/): the `IPlugin` contracts you implement in-process
- [Save games](/save-games/): the checkpoint format, the WAL, and what survives a crash
- [Architecture](/architecture/): where the engine sits relative to the REST API and the UI
