...
| `Fallen8:Agents:TimeoutSeconds` | `30` | the small routes, and the wait for the feed's response headers; the feed's body takes none of it, so a stream stays open as long as its subscriber wants |
