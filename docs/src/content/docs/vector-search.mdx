...
answer under `/ns/{ns}/…` for a specific namespace ([namespaces.md](/namespaces/)).
...
A vector index is created through the normal index surface ([indexes.md](/indexes/)):
...
| `embeddingName` | no | Binds the index to a named element embedding: the index then maintains itself and rejects explicit adds ([semantic-traversal.md](/semantic-traversal/)) |
| `model` | no | Opaque model-identity string, stored and persisted; enforced by the embedding provider ([semantic-traversal.md](/semantic-traversal/)) |

The example below creates a **raw** index with a tiny dimension, to keep the vectors readable. For
real work you almost always want the **bound** form instead - add `embeddingName`, take `dimension`
and `metric` from the `embedding` block of `GET /status`, and the index maintains itself from element
state. In F8 Studio the create form prefills both from the instance's provider for exactly that
reason: an index whose dimension disagrees with the model writing into it is refused on every later
embed and every search. Order does not matter, since a bound index created after the vectors exist
materialises itself from them.
...
maintains for you ([semantic-traversal.md](/semantic-traversal/)).
...
[semantic-traversal.md](/semantic-traversal/)); `404` for an unknown index or element.
...
([indexes.md](/indexes/)). Its sibling `DELETE /index/{indexId}/propertyValue` cannot
...
| `label` | no | Exact, case-sensitive label match on the element's own label; unlabeled elements never match. Never an edge's type ([edge type vs label](/graph-model/#edge-type-vs-label)), and there is no edge-type restrictor |
...
provider): see [semantic-traversal.md](/semantic-traversal/). F8 Studio's Query screen offers
both: a pasted vector under **ask an index**, and typed text under its own **semantic search**
mode, which also offers to create the bound index when the instance has none
([studio.md](/studio/)).

## From hits to graph (GraphRAG)

kNN hits are graph elements, so retrieval does not stop at a ranked list: connect two hits
with a path query ([path-finding.md](/path-finding/)), expand a hit's neighbourhood as a
subgraph ([subgraphs.md](/subgraphs/)), or read its properties and adjacency directly
([graph-model.md](/graph-model/)), then feed the retrieved neighbourhood, not isolated
snippets, to the model. To rank or filter *during* a traversal instead of before it, use
semantic traversal ([semantic-traversal.md](/semantic-traversal/)).
...
being ordinary vertex ids ([unstructured-ingestion.md](/unstructured-ingestion/)).
...
Indices live in checkpoints ([save-games.md](/save-games/)) and nowhere else: neither
...
re-created (the family-wide rule is in [indexes.md](/indexes/)). Once the index exists
again, a *bound* one re-projects itself from the WAL-covered element embeddings
([semantic-traversal.md](/semantic-traversal/)); an *unbound* one has lost every vector
...
- [indexes.md](/indexes/): the index surface all families share
- [semantic-traversal.md](/semantic-traversal/): element embeddings, bound indices, the embedding provider, semantic path/subgraph blocks
- [unstructured-ingestion.md](/unstructured-ingestion/): the semantic layer (documents in, graph out) over its own bound vector index
- [path-finding.md](/path-finding/) / [subgraphs.md](/subgraphs/): the traversal surface kNN hits feed into
