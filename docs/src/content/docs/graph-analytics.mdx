...
description: "PageRank, connected components, communities, degree centrality, and triangle counting, with optional property write-back."
---
...
data (labels, numbers, flags): no [delegates](/fallen-8-core/delegates/) and no dynamic code, so
analytics compiles no C# at all (see [security.md](/fallen-8-core/security/)).
Analytics algorithms are the third plugin family next to path traversers and subgraph
algorithms, discovered the same way (see [plugins.md](/fallen-8-core/plugins/)).

## Endpoints

| Route | What it does |
|---|---|
| `GET /analytics/algorithms` | Lists the available algorithm plugins (name → description) |
...
[namespaces.md](/fallen-8-core/namespaces/)).
...
case must match the registration ([plugin-registration.md](/fallen-8-core/plugin-registration/)).
...
| `edgePropertyId` | all | all edges | Only edges in this adjacency group are traversed ([edge type vs label](/fallen-8-core/graph-model/#edge-type-vs-label)) |
...
| Field | Meaning |
|---|---|
| `partitionId` | The requested partition, echoed back |
| `size` | The partition's total size across all pages: the paging terminator |
| `offset` | The requested offset, echoed back |
| `members` | This page's member vertex ids, ascending |

A partition id the run did not produce is a 404, and so is any id when the scope was empty
(the run then produced no partitions at all).
...
gone (re-run to restore). See [save-games.md](/fallen-8-core/save-games/).

A write-back is opaque to the [change feed](/fallen-8-core/change-feed/): each chunk publishes
...
# => "properties" now contains (read DTOs stringify every value, invariant culture):
#    { "propertyId": "analytics.pagerank", "propertyValue": "0.0031", "fullQualifiedTypeName": "System.Double" }
```
...
- [delegates.md](/fallen-8-core/delegates/): why queries are C# delegates and analytics deliberately are not
- [plugins.md](/fallen-8-core/plugins/): the plugin system that discovers analytics algorithms
- [graph-model.md](/fallen-8-core/graph-model/): vertices, properties and transactions
- [path-finding.md](/fallen-8-core/path-finding/), [subgraphs.md](/fallen-8-core/subgraphs/): the sibling algorithm families
- [save-games.md](/fallen-8-core/save-games/): snapshots, WAL and why write-back is snapshot-durable only
- [observability.md](/fallen-8-core/observability/): graph statistics beyond per-run aggregates
