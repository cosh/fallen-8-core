...
|---|---|---|---|
| `pathAlgorithmName` | string | `BLS` | `BLS`, `DIJKSTRA` (below), or the name of a registered `Path` algorithm plugin ([plugin-registration.md](/fallen-8-core/plugin-registration/)). Matched case-sensitively; an unknown or mis-cased name yields an empty result, not an error. |
...
| `storedQuery` | string | (none) | Invoke a registered `Path` query by name instead of inline fragments, mutually exclusive with `filter`/`cost` ([stored-queries.md](/fallen-8-core/stored-queries/)). |
| `semantic` | object | (none) | Code-free similarity filter/cost block ([semantic-traversal.md](/fallen-8-core/semantic-traversal/)). |
| `timeBudgetSeconds` | double | unbounded | Cooperative deadline for the traversal; `408` when it is exhausted, and the request is always bound to the client's abort as well. Checked between filter and cost invocations, so it bounds an expensive traversal but cannot interrupt a single fragment that never returns ([security](/fallen-8-core/security/#dynamic-code-execution-always-on)). |

## Algorithms

Both traverse edges in **both directions** (undirected reachability over directed edges); each step records the direction it actually used. Both are discovered as plugins ([plugins.md](/fallen-8-core/plugins/)); `GET /status` → `availablePathPlugins` reports the invocable set, which is the two built-ins plus any `Path` algorithm plugin registered in this namespace.
...
These fragments are compiled at runtime with Roslyn and run in-process; the full contract, the accessor surface, and the `POST /delegates/validate` compile-check live in [delegates.md](/fallen-8-core/delegates/). Dynamic code execution is always on, so a request carrying inline fragments just needs the credential when a key is configured ([security.md](/fallen-8-core/security/)). Two code-free alternatives exist for callers that never compile C#: a `storedQuery` reference ([stored-queries.md](/fallen-8-core/stored-queries/)) and the `semantic` block ([semantic-traversal.md](/fallen-8-core/semantic-traversal/)).
...
| `edgePropertyId` | string | The edge's type: the adjacency group it sits under ([edge type vs label](/fallen-8-core/graph-model/#edge-type-vs-label)) |
...
Given three vertices where `A → B` weighs 10 while `A → C` and `C → B` each weigh 1, the cheapest `A → B` route is the two-hop `A → C → B` (total 2), not the direct weight-10 edge. That graph needs building first. Store `weight` as `System.Double`, because `TryGetProperty<double>` treats a value of any other CLR type as absent and the fragment below would then fall back to `1.0` for every edge ([delegates.md](/fallen-8-core/delegates/)). The two waited-on batch calls return the assigned ids; continuing in the graph from the example above they come out as `[11, 12, 13]` for `A`, `B`, `C` and `[14, 15, 16]` for the three roads:
...
| `401` | No credential supplied while a key is configured ([security.md](/fallen-8-core/security/)) |
...
A request that carries `semantic.queryText` can additionally answer `403` or `503` when the embedding provider is off or unavailable ([semantic-traversal.md](/fallen-8-core/semantic-traversal/)).

## See also

- [Delegates](/fallen-8-core/delegates/): the no-query-language philosophy, the fragment contract, compilation, and `/delegates/validate`
- [Stored queries](/fallen-8-core/stored-queries/): precompiled `Path` queries invoked by name, no dynamic code required
- [Semantic traversal](/fallen-8-core/semantic-traversal/): the code-free `semantic` similarity block carried on `/path`
- [Security](/fallen-8-core/security/): the API key that gates access to these always-on code endpoints
- [Graph model](/fallen-8-core/graph-model/): vertices, edges, properties, and the transactions that build a graph
- [Samples](/fallen-8-core/samples/): weighted datasets to traverse (air-routes `km`, attack-surface `exploitCost`)
- [Plugins](/fallen-8-core/plugins/): how path algorithms are discovered
- [Plugin registration](/fallen-8-core/plugin-registration/): adding your own `Path` algorithm, invocable by name through `pathAlgorithmName`
- [Namespaces](/fallen-8-core/namespaces/): per-namespace routing (`/ns/{ns}/…`)
