...
| `storedQuery` | string | — | Invoke a registered `Path` query by name instead of inline fragments — mutually exclusive with `filter`/`cost` ([stored-queries.md](/fallen-8-core/stored-queries/)). |
| `semantic` | object | — | Code-free similarity filter/cost block ([semantic-traversal.md](/fallen-8-core/semantic-traversal/)). |

## Algorithms

Both traverse edges in **both directions** (undirected reachability over directed edges); each step records the direction it actually used. Both are discovered as plugins ([plugins.md](/fallen-8-core/plugins/)) — the shipped set is reported by `GET /status` → `availablePathPlugins`.
