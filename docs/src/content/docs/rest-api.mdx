...
| Namespaces | Every namespace-scoped route also answers under `/ns/{name}/…`; bare URLs address the reserved `default` namespace. A namespace that exists but was not loaded by this process refuses every one of them with `503` before the action runs (`namespaceState: "notLoaded"`); `GET /status` is the single exception and reports that state instead. See [namespaces](/fallen-8-core/namespaces/#startup-load). |
...
| Benchmark **(Fallen-8-level)** | `GET /generate`, `GET /benchmark` | [benchmark.md](/fallen-8-core/benchmark/) |
