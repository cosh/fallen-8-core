...
It describes the addressed namespace and also answers under `/ns/{ns}/statistics` ([namespaces](/fallen-8-core/namespaces/)), so the bare URL below is the reserved `default` graph. Counts, cardinalities and the index inventory are per-namespace; the `memory` block is process-wide.
