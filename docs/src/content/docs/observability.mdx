...
**App meter `NoSQL.GraphDB.App`** (process-wide; the Roslyn codegen signals, see [delegates](/fallen-8-core/delegates/)):
...
It describes the addressed namespace and also answers under `/ns/{ns}/statistics` ([namespaces](/fallen-8-core/namespaces/)), so the bare URL below is the reserved `default` graph. A namespace this process did not load refuses it with `503`, like every namespace-scoped route except `/status`. Counts, cardinalities and the index inventory are per-namespace; the `memory` block is process-wide.
...
Fleet observability itself spans both roles. On the **producer** side each apiApp and [MCP server](/fallen-8-core/mcp-server/) stamps a fleet identity on every signal and adds the per-action signals a fleet dashboard needs. On the **consumer** side a small stack (OpenTelemetry Collector, Prometheus, Tempo, Loki, Grafana) ingests the push and renders it.
...
- [Security](/fallen-8-core/security/): whether `/metrics` and `/statistics` require the API key, the rate limiter that guards `/statistics`, and the deployment postures
- [Save games](/fallen-8-core/save-games/): what the WAL / checkpoint / degraded-durability metrics mean
- [Delegates](/fallen-8-core/delegates/): the codegen cache the `fallen8.codegen.*` metrics track
- [Semantic traversal](/fallen-8-core/semantic-traversal/): the `embedding` block on `/status` and `/statistics`
- [MCP server](/fallen-8-core/mcp-server/): the per-tool telemetry source for the fleet
- [Running Fallen-8](/fallen-8-core/running/): configuring the server and compose, and the one-command fleet environment
- [REST API](/fallen-8-core/rest-api/): the full endpoint surface
- [Studio](/fallen-8-core/studio/): the dashboard that reads `/status` and `/statistics`
- [Architecture](/fallen-8-core/architecture/): how the observability stack fits the whole
