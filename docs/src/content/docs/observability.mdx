...
- `availableIndexPlugins`, `availablePathPlugins`, `availableAnalyticsPlugins`, `availableServicePlugins`: the discovered [plugins](/fallen-8-core/plugins/).
...
- `embedding`: the embedding provider state; reading it never triggers the lazy model load. See [semantic traversal](/fallen-8-core/semantic-traversal/).
...
- [Security](/fallen-8-core/security/): whether `/metrics` and `/statistics` require the API key, the tag-hygiene invariant, and the trust posture
