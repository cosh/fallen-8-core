...
description: "Embeddings as element state; a code-free semantic block steers paths and subgraphs by similarity."
---
...
philosophy lives in [delegates.md](/delegates/)). Every graph-scoped route
here also answers under `/ns/{ns}/…` for a specific graph ([namespaces.md](/namespaces/));
...
3. **Scores are the kNN scores.** The same `VectorMath` backs both, so a traversal score is
   bit-identical to [`/scan/index/vector`](/vector-search/). Traversal needs **no** vector
...
([save-games.md](/save-games/)): one current vector per (element, name); a write replaces,
`DELETE` removes.
...
- Physically the vector sits on the reserved property key `$embedding:<name>`, the provider stamp
  on `$embeddingModel:<name>`. Element reads return both in the ordinary `properties` list
  (`GET /vertex/{id}`, `GET /edge/{id}`, `GET /graphelement/{id}`, `GET /graph`), so an embedded
  element's payload inlines the whole vector: 1024 floats with the compose default.
- On the JSONL wire the embedding travels as its reserved typed property: see
  [bulk-import-export.md](/bulk-import-export/).
...
A `VectorIndex` created with the `embeddingName` option ([vector-search.md](/vector-search/)
...
- **No explicit adds**: `PUT /index/vector/{id}` answers 400; write the element embedding
  instead.
- **No explicit removals either**: the generic index-content removals
  (`DELETE /index/{indexId}/{graphElementId}`, `DELETE /index/{indexId}/propertyValue`) answer 400 on
  a bound index, for the same reason as the add. Removing the element's embedding is the way to drop
  it from the projection, which keeps membership under one authority.
...
| `costBySimilarity` | bool | **Path only**: installs a vertex cost, `Cosine`→`1-score`, `L2`→distance; without `minScore` it also fills the **vertex-filter** slot with an implied has-embedding filter (a cost is only defined over embedded vertices) |
...
| Request | Answer |
|---|---|
| `minScore` (or `costBySimilarity` without `minScore`) + a vertex-filter fragment (inline or stored) | 400 |
...
the provider present ([subgraphs.md](/subgraphs/)).
...
- A vertex pattern step's `semanticMinScore` fills **that step's** filter slot, so a fully
  ...
  Setting it together with the step's `vertexFilter`, on an edge step, non-finite, or without a
  request-level `semantic` block is a 400. `costBySimilarity` is rejected (a path concept).
- Stored templates carry no semantics: a `semantic` block on a stored-template invocation is a 400
  ([stored-queries.md](/stored-queries/)), and `semanticMinScore` inside a stored
  `SubGraph` template is rejected already at registration (`POST /storedquery`), because a
  template's delegates bind before any query vector exists. Inline the filters instead.
...
| `Backend` | `Onnx` | `Onnx` \| `LLamaSharp` \| `Ollama` \| `Nahil` \| `OpenAI`: the whole backend swap, and [which to pick](/model-providers/) |
| `ModelName` / `ModelVersion` | (none) | Identity name (required when enabled) + optional version |
| `Dimension` | (none) | Declared output length; a mismatch against real output latches 503 |
| `IntendedMetric` | `Cosine` | Metric the vectors are meant for |
| `MaxBatchSize` / `MaxTextLength` | 64 / 8192 | Request bounds, in items and **chars**. `MaxTextLength` is not a token window - see below |
| `MaxConcurrentBatches` | `1` | How many `MaxBatchSize` requests [document ingestion](/unstructured-ingestion/) keeps in flight at once for one document; raise to 2, then 4 or 8, only while p95 latency and the failure rate stay acceptable |
| `QueryPrefix` | `""` | Retrieval prefix applied to **query-time** embeddings only |
| `Onnx.{ModelPath,VocabPath,…}`, `LLamaSharp.ModelPath`, `Ollama.{Endpoint,Model}`, `Nahil.{…}`, `OpenAI.{Endpoint,ApiKey,Model}` | (none) | Backend-specific settings |

The provider's biggest consumer is [unstructured ingestion](/unstructured-ingestion/):
document chunks embed through it on the way into the graph.

### The input ceiling (2048 tokens, not 8192)

`bge-m3` reports an 8192-token context and **nothing serves it**. Measured: an input of about
1,880 tokens answers `200`, one of about 2,120 answers `400`. The real ceiling is **2048 tokens
per input**, and it is the same on the local Ollama sidecar and on [Nahil](/nahil/) - so this is a
...
- **Fallen-8 sends `truncate: false` on every embedding request.** An input over the ceiling is
  refused, with a `503` naming the ceiling and which setting to lower. A failed ingest is
  re-runnable; a silently truncated vector is not even visible.
- **`Fallen8:Ingestion:ChunkMaxChars` defaults to 3,600** (it was 4,000, derived from the advertised
  8192). That is a token budget wearing a char unit: it holds a chunk under ~1,800 tokens at
  2.0 chars/token, the worst case measured over markdown tables (2.10), ARXML (2.23) and
  punctuation-dense text (2.04).

Without the second, the first would turn silent degradation into failed ingests - which is why the
default moved and not just the flag.

To be exact about the old value rather than alarmed about it: 4,000 was *inside* the ceiling for
every Latin-script, table and XML sample measured - but by under 70 tokens at the densest of them
(~1,980 of 2,048), which is no margin at all for a denser page - and already *outside* it for
Korean (~2,400) and Chinese (~3,010). 3,600 buys ~250 tokens of margin and costs prose nothing.
...
The embedding provider has a sibling under the same "semantic gateway" idea: a **chat
...
NL-assist transport in [F8 Studio](/studio/), also apiApp-only and capability-gated, and the
...
[model providers](/model-providers/)), `Ollama.Endpoint`, one model per purpose (below), and
...
provider and Chat sections of the [configuration surface](/configuration/) behind it. The values that
...
The embedding provider has the same knob, `Fallen8:Embedding:TimeoutSeconds`, defaulting to 300
seconds because one call embeds a whole batch of texts; an exceeded batch answers `503`, like any
other "backend not usable right now". It is the smaller of the two: the chat budget is 600 because
that capability's default backend may spend it waiting for a cold model to be pulled. On a CPU-only host both defaults are generous and still may not be
enough: see [NL assist spins on "generating"](/troubleshooting/#nl-assist-spins-on-generating-then-gives-up-504)
for what CPU inference actually costs.

### One model per purpose

A completion can say what it is **for**, and the server picks the model for that job. The request
carries `purpose`, either `assist` (the default) or `agent`, and each backend block names one model
per purpose:

```
Fallen8__Chat__Ollama__Models__Assist=phi4-f8-mini:latest
Fallen8__Chat__Ollama__Models__Agent=phi4-mini:latest
```

The reason is that the two jobs want opposite models. NL assist wants the fine-tune that emits a
single C# fragment and nothing else; an agent wants a general model that calls tools and holds a
conversation. One setting pointed at both means one of them is wrong.

A purpose names a **job, never a model**, so no client chooses what runs: it selects a name the
instance owns. Omit the field and nothing changes, which is why this was an addition rather than a
break. A value that is neither name is refused with `400` listing both, rather than quietly serving
the assist model to an agent. A purpose whose model nobody configured answers `503` naming the key
to set, and the other purpose keeps working. `GET /config` and `GET /status` report both models,
with `model` still meaning the assist one.

The two metered providers ship with **neither** model set, because a model nobody named is a model
nobody agreed to pay for. The local sidecar and Nahil default both purposes to models the shipped
environment already pulls.

### Tool calling

`POST /chat` can offer the model **tools**, which is what an
[agent](/mcp-server/) run needs. Send `tools` alongside the messages, each with a name, a sentence
saying when to use it, and a JSON Schema for its arguments:

```json
{
  "purpose": "agent",
  "messages": [{ "role": "user", "content": "how many vertices?" }],
  "tools": [{
    "name": "count_vertices",
...
The model answers with `toolCalls` instead of text, each carrying an `id`, a `name` and the
`arguments` it produced. **You run them.** Fallen-8 executes nothing, remembers nothing and holds no
conversation state: send the results back on the next request as `tool` messages carrying the
matching `toolCallId`, alongside the `assistant` message that made the calls. That assistant message
has no `content`, which is legal precisely because it is how a tool-calling turn looks.

Three things worth knowing before you build on it:

- **Whether the model can call tools at all is the model's property**, not the field's. The assist
  fine-tune cannot, which is what the `agent` purpose exists for.
- **A request carrying tools is not streamed**, on any backend. That is a client-library
  constraint rather than a wire one, and it affects nothing else.
- **The schema reaches the provider as written.** Nothing validates it, and nothing validates the
  arguments a model produces against it either, because you run the tool and are the only party
  that knows what its arguments mean.
...
| `Nahil` | no | the same `bge-m3`, on nahil.dev | no weights on the host, and no vector re-embeds ([Nahil](/nahil/)) |
| `OpenAI` | no | none of yours | a **different** embedding function: new stamp, new dimension, so it never moves with a chat switch ([model providers](/model-providers/)) |
...
`F8_EMBEDDINGS=false` ([running.md](/running/)).

Endpoints (all 403 while disabled; 413 over 1 MiB; 429 on the sensitive-endpoint rate limit;
502 for bad backend output; 503 when the backend is unavailable):
...
load; [observability.md](/observability/)). F8 Studio gates its text-in controls on it
([studio.md](/studio/)).
...
- [model-providers.md](/model-providers/): which backend serves embeddings and chat, and what a switch moves
- [vector-search.md](/vector-search/): VectorIndex kNN mechanics (metrics, brute force, `/scan/index/vector`)
- [delegates.md](/delegates/): the no-query-language philosophy and the `context` parameter for compiled fragments
- [path-finding.md](/path-finding/) / [subgraphs.md](/subgraphs/): the traversal surfaces the block rides
- [security.md](/security/): the API key (dynamic code is always on)
- [stored-queries.md](/stored-queries/): pre-compiled fragments that can read `context`
- [samples.md](/samples/): embedded datasets to try semantic queries against
- [namespaces.md](/namespaces/): per-namespace routing (`/ns/{ns}/…`)
