...
philosophy lives in [delegates.md](/fallen-8-core/delegates/)). Every graph-scoped route
here also answers under `/ns/{ns}/…` for a specific graph ([namespaces.md](/fallen-8-core/namespaces/));
...
3. **Scores are the kNN scores.** The same `VectorMath` backs both, so a traversal score is
   bit-identical to [`/scan/index/vector`](/fallen-8-core/vector-search/). Traversal needs **no** vector
...
([save-games.md](/fallen-8-core/save-games/)): one current vector per (element, name); a write replaces,
`DELETE` removes.
...
- On the JSONL wire the embedding travels as its reserved typed property: see
  [bulk-import-export.md](/fallen-8-core/bulk-import-export/).
...
A `VectorIndex` created with the `embeddingName` option ([vector-search.md](/fallen-8-core/vector-search/)
...
[delegates.md](/fallen-8-core/delegates/).
...
the provider present ([subgraphs.md](/fallen-8-core/subgraphs/)).
...
- Stored templates carry no semantics: a `semantic` block on a stored-template invocation is a 400
  ([stored-queries.md](/fallen-8-core/stored-queries/)), and `semanticMinScore` inside a stored
...
The provider's biggest consumer is [unstructured ingestion](/fallen-8-core/unstructured-ingestion/):
...
NL-assist transport in [F8 Studio](/fallen-8-core/studio/), also apiApp-only and capability-gated, and the
...
`F8_EMBEDDINGS=false` ([running.md](/fallen-8-core/running/)).
...
load; [observability.md](/fallen-8-core/observability/)). F8 Studio gates its text-in controls on it
...
- [vector-search.md](/fallen-8-core/vector-search/): VectorIndex kNN mechanics (metrics, brute force, `/scan/index/vector`)
- [delegates.md](/fallen-8-core/delegates/): the no-query-language philosophy and the `context` parameter for compiled fragments
- [path-finding.md](/fallen-8-core/path-finding/) / [subgraphs.md](/fallen-8-core/subgraphs/): the traversal surfaces the block rides
- [security.md](/fallen-8-core/security/): the API key (dynamic code is always on)
- [stored-queries.md](/fallen-8-core/stored-queries/): pre-compiled fragments that can read `context`
- [samples.md](/fallen-8-core/samples/): embedded datasets to try semantic queries against
