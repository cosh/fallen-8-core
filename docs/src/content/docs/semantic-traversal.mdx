...
in Studio's Connect **Configuration** section (`GET /config`). The values that identify a model are
deliberately **not** editable there: the model name, version, dimension and metric are stamped beside
every vector you have already stored, so changing one would mislabel that data rather than fail
