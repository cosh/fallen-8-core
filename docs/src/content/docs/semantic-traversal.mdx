...
| `Backend` | `Onnx` | `Onnx` \| `LLamaSharp` \| `Ollama` \| [`Nahil`](/nahil/): the whole backend swap |
...
| `Onnx.{ModelPath,VocabPath,…}`, `LLamaSharp.ModelPath`, `Ollama.{Endpoint,Model}` | (none) | Backend-specific settings |
...
`F8_CHAT`): `Backend` (`Ollama` or `Nahil`, see [Nahil](/nahil/)),
`Ollama.{Endpoint,Model}` (default `phi4-f8-mini:latest`),
