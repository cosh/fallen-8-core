...
| import | `200` | Completed; the body is the created-counts summary plus `durable` |
...
- **Durability is reported, not assumed.** That fsync holds while the log is healthy. If the log is degraded (a tripped failure fence, or one awaiting its paired snapshot load) every batch still commits **in memory**, so the counts stay true, but those batches are not in the log and a kill would lose them. The response therefore carries `durable`: `false` means take a checkpoint ([save games](/save-games/)) before trusting the import to survive a restart. `GET /status` carries the durability detail.
...
```json
{"verticesCreated":34,"edgesCreated":78,"linesRead":113,"durable":true}
```

`linesRead` counts every line, including the meta line and blank lines (blank lines are tolerated). `durable` is `true` in the normal case and whenever the write-ahead log is disabled (durability is then by explicit checkpoint); see the durability note above for what `false` means. The `400` problem body carries the same flag next to its committed counts.
