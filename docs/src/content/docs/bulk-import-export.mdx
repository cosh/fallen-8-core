...
`GET /bulk/export` takes optional `vertexLabel` and `edgeLabel` query parameters — exact-match label filters passed straight to the engine's scans. Both endpoints are per-namespace like every data route: a bare URL addresses `default`, `/ns/{name}/bulk/…` addresses a named graph ([namespaces](/fallen-8-core/namespaces/)). Both sit behind the standard API-key fallback policy ([security](/fallen-8-core/security/)).
