...
`GET /bulk/export` takes optional query parameters, all exact-match: `vertexLabel` and `edgeLabel` filter by label, `edgePropertyId` filters edges by their type (it combines with `edgeLabel` as AND, [edge type vs label](/fallen-8-core/graph-model/#edge-type-vs-label)). Both endpoints are per-namespace like every data route: a bare URL addresses `default`, `/ns/{name}/bulk/…` addresses a named graph ([namespaces](/fallen-8-core/namespaces/)). Both sit behind the standard API-key fallback policy ([security](/fallen-8-core/security/)).
...
F8 Studio drives both endpoints from the **interchange (jsonl)** block on the Save games screen, with the same label and edge-type filters on export ([studio](/fallen-8-core/studio/)).
...
Two details make that more than a formality on the index side. A restore normally starts from zero indexes, because `HEAD /tabularasa` drops every index *definition*, not just its entries. And re-creating a key-based index (dictionary, single-value, range, regex) does not restore its content: it comes up empty and has to be refilled with `PUT /index/{indexId}`, one element and key at a time ([indexes](/fallen-8-core/indexes/)). The one index that needs nothing is a `VectorIndex` bound to an embedding name: it projects itself from the imported `$embedding:<name>` properties at creation time.

For durable point-in-time backups of a running instance, use checkpoints instead ([save games](/fallen-8-core/save-games/)).

## See also

- [Graph model](/fallen-8-core/graph-model/): elements, properties, and the property type allow-list
- [Namespaces](/fallen-8-core/namespaces/): the `/ns/{name}/…` routing these endpoints follow
- [Save games](/fallen-8-core/save-games/): checkpoints, the write-ahead log, the other durability path
- [Indexes](/fallen-8-core/indexes/): what an import does *not* rebuild, and how to refill it
- [Studio](/fallen-8-core/studio/): the interchange (jsonl) block that drives both endpoints from the UI
- [Semantic traversal](/fallen-8-core/semantic-traversal/): element embeddings, carried here as `System.Single[]`
- [Security](/fallen-8-core/security/): the API key that gates both endpoints
- [Samples](/fallen-8-core/samples/): the bundled sample datasets ship in this format
