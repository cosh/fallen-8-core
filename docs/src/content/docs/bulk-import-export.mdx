...
| `edgePropertyId` | edge | Required non-empty string; the edge's type — the adjacency-list key ([edge type vs label](/fallen-8-core/graph-model/#edge-type-vs-label)) |
...
```bash
# Export the whole graph. Add ?vertexLabel=member&edgePropertyId=interactsWith for a subset.
