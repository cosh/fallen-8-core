---
title: "Indexes"
description: "Dictionary, range, fulltext, spatial R-Tree, and vector kNN index types, all as plugins."
---
...
| Add a vector (vector family) | `PUT /index/vector/{indexId}` | see [vector-search.md](/fallen-8-core/vector-search/) | `true`/`false` |
...
rejects explicit adds ([semantic-traversal.md](/fallen-8-core/semantic-traversal/)).
...
([graph-model.md](/fallen-8-core/graph-model/)) or a compiled delegate ([delegates.md](/fallen-8-core/delegates/)).
...
- [delegates.md](/fallen-8-core/delegates/) — why there is no query language: queries are compiled C# delegates
