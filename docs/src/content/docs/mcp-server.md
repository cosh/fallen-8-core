...
| `Fallen8Target:TimeoutSeconds` | deadline on one bridged REST call (default `330`, clamped to `1`..`2147483`: a non-positive value is a one-second deadline rather than none, and a very large one is capped at what the transport can be armed with rather than throwing). Exceeding it returns the bridge's retryable `504` "Fallen-8 timeout". The default sits above the instance's own longest budget on a bridged route (`Fallen8:Embedding:TimeoutSeconds`, 300s, which a semantic search spends embedding the query) so the instance's error wins instead of being cut off here |
