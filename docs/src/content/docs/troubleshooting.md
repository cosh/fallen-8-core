...
1. **There is no vector index at all.** Embeddings are element state, so a run can write
   thousands of them while nothing exists to rank them: the Indexes screen then shows only
   other families and reads as "this instance cannot search by meaning". Studio now says so
   where it matters. The Query screen's **semantic search** mode offers the create in place,
   prefilled from the provider's own dimension and metric, and the Indexes screen names what is
   missing. Over REST, look for a `VectorIndex` in the `indices` block of `GET /status`.
2. **Nothing was ever embedded.** For an integration run, the embed opt-in is off by default -
...
3. **The index is bound to a different name.** A bound index only ever projects the one embedding
   name it declares. The Indexes screen shows it as `bound:<name>`, and so does the semantic
   mode's index picker; if the run wrote `default` and the index binds `arxml-summary`, neither
   is wrong and they will never meet.
4. **The label constraint excludes everything.** Constraints are applied *before* scoring, so a
   label nothing carries yields nothing rather than the unconstrained answer. Drop the `label`
   and see whether hits appear.
5. **The index is raw, not bound.** An unbound index holds only vectors somebody added
