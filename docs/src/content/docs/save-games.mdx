...
| `PUT /save/all` | Fallen-8 | (none) | `200` one entry spanning all namespaces · `429` rate-limited · `500` (body names failed namespaces; successful ones are still registered) |
...
| Field | Means |
|---|---|
| `walEnabled` | a write-ahead log is open for this namespace |
...
| `PUT /savegames/{id}/load` | Restore the entry's namespaces, replacing their in-memory graphs | `200` (waited) · `202` (accepted) · `404` · `500` (three causes, below) |
