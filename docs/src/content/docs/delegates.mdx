...
Compile-checks one fragment without running it — nothing is emitted, loaded, or executed. Use it to give an editor or agent instant feedback before submitting a query. It carries the same authentication as the query endpoints ([security.md](/fallen-8-core/security/)): `401` without a credential when a key is configured, `429` when rate-limited.
