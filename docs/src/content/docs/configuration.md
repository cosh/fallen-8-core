...
First, **a never-writable setting publishes no value**. The key, its tier, the rule and the reason, yes; the value, no. That is what keeps sidecar addresses, model file paths and durability paths out of a response an unauthenticated caller could read.
