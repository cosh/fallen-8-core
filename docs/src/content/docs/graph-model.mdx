...
`POST /scan/graph/property/...`. Index-backed scans live in [Indexes](/indexes/); C# query
delegates in [Delegates](/delegates/).
...
vector state; see [Semantic traversal](/semantic-traversal/). They are not filtered
...
Writer/commit internals are in [Architecture](/architecture/).

## REST CRUD

Base URL `http://localhost:8080`. These are the bare (default-namespace) paths; every route also
answers under `/ns/{name}/...`; see [Namespaces](/namespaces/).
...
| `HEAD /tabularasa` | Erase all data in the addressed namespace (it stays registered, empty). Every index **definition** goes with it, not just its entries ([Indexes](/indexes/)). |
...
- **Removing a vertex removes its edges.** The cascade deletes every incident edge, so
  `GET /edge/count` drops with it, and the [change feed](/change-feed/) reports one
...
- **`GET /graph` bounds vertices and edges independently.** `maxElements` is applied to each of the
  ...
  `GET /bulk/export` ([Bulk import/export](/bulk-import-export/)).
...
loading and dumping are covered in [Bulk import/export](/bulk-import-export/).
...
indexed lookups use [Indexes](/indexes/). Request body:
...
builds is listed under [Path finding](/path-finding/)), which stores each person's
...
- [Indexes](/indexes/): index types and `POST /scan/index/...`.
- [Delegates](/delegates/): why there is no query language; C# predicates over the graph.
- [Path finding](/path-finding/): traversals between vertices.
- [Namespaces](/namespaces/): isolated graphs and `/ns/{name}` routing.
- [Semantic traversal](/semantic-traversal/): element embeddings and reserved property keys.
- [Bulk import/export](/bulk-import-export/): creating and dumping graphs in bulk.
- [REST API](/rest-api/): versioning, OpenAPI, and shared conventions.
