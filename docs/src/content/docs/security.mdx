...
Plugin registration is one of six operator **capability** switches the authorization layer enforces. A capability is orthogonal to authentication: when it is off the endpoint refuses every caller, valid key-holder or not, and which refusal you get says what kind of instance you are talking to. With an API key configured it is **403**; with none it is **401**, because the policy has no authenticated caller to forbid. Plugin registration is the one switch that is on by default, so the 401 is what a bare run answers for the other five.
...
| Embedding / chat / ingestion capabilities | `Fallen8:Embedding:Enabled`, `Fallen8:Chat:Enabled`, `Fallen8:Ingestion:Enabled` | `/embedding/*`, `POST /chat` **and** `GET /chat/models`, `/document/*` (each group is also the only path to an external sidecar; the catalog read is the one route that proxies a credentialed backend's model *list*: [which models the backend has](/model-providers/#which-models-the-backend-has)) | `false` → refused on a bare run (**401** while no key is configured, **403** once one is); the compose environment turns all three on (see [running](/running/)) |
| Integrations / agents capabilities | `Fallen8:Integrations:Enabled`, `Fallen8:Agents:Enabled` | `/integrations/*` and `/agents/*`, the authenticated proxies to the two separate deployables that publish no port of their own (see **Two more listeners** below) | `false` on a bare run; the compose environment turns integrations on and leaves agents off until `F8_AGENTS=true`, alone among these switches, because an agent decides for itself which tools to call |
...
**Two more listeners hold credentials and authenticate nobody.** The [integrations runtime](/integrations/) and the [agent host](/agents/) are separate deployables, and unlike the MCP server they publish **no host port at all**, so from your machine or a browser the only way in is the API's authenticated proxy at `/integrations/*` and `/agents/*`. Inside the compose network the honest picture is different: every service on `f8-net` can reach both, neither asks a caller for anything, and each holds `F8_API_KEY` (the agent host also holds the MCP bearer). So a service on that network can submit an integration job, or spawn an agent at whatever MCP tiers you enabled, without presenting a credential. That is a deliberate convention for these two rather than an oversight, and the unpublished port is what it rests on: the network is the boundary. Keep `f8-net` to services you run, do not publish `8110` or `8120` "to debug it", and note that setting `F8_API_KEY` closes neither of them. The agent host says its own bind out loud at startup and warns when it is not loopback.
...
| `SensitiveRateLimitPermitPerWindow` / `RateLimitWindowSeconds` | `30` / `10` | Fixed-window rate limit; breach → **429** with no queueing. The window is **process-wide, not per caller**: 30 requests from any one client inside 10 s rate-limit everyone. The guarded set is wider than the code endpoints: `POST /path/…`, `PUT /subgraph`, `POST /storedquery`, `POST /delegates/validate` and `POST /plugins/*`, plus `GET /statistics`, namespace create / rename / drop, `PUT /save/all`, `HEAD /tabularasa/all`, `POST /chat`, `GET /chat/models` (guarded because one read fans out to the backend rather than making a single request: [how wide](/model-providers/#which-models-the-backend-has)), every `/embedding/*` route, and `POST /document`, `/document/text`, `/document/search`. |
| (request body) | 1 MiB | Fixed per endpoint (a `[RequestSizeLimit]` attribute), not configurable: a fragment or registration body over it → **413**. There is deliberately no configuration key for it. Two endpoints carry their own, much larger bound because a file travels in the body: document upload ([unstructured ingestion](/unstructured-ingestion/)) and `POST /integrations/job` ([integrations](/integrations/#files)). For the latter the bound is 768 MiB, set above any legal job on purpose, and it is judged from the declared `Content-Length` **before the body is uploaded**: a body over it answers **413** naming both numbers, having read nothing, and a body sent without a `Content-Length` answers **411**, because one that cannot be measured cannot be refused before it arrives. The real per-file ceiling remains the *runtime's* `Integrations:MaxFileBytes`, which refuses a legal-sized body with a message naming both sizes. `POST /agents` carries the plain 1 MiB bound, and the [agent host](/agents/) behind it bounds `task`, `name` and `systemPromptAppendix` separately, because one legal body can carry all of it in one field. |
