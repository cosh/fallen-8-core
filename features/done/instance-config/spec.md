...
1. **Semantic gateway (server).** Fallen-8 already embeds text server-side
   (`POST /embedding/text` → Ollama/Onnx/LLamaSharp). This feature adds the missing half — a
   **server-side chat proxy** (`POST /chat`) forwarding to the Ollama sidecar — so the
...
2. **Default routing through F8.** Studio's NL-assist (delegate/plugin drafting) reroutes to
   **browser → Fallen-8 REST → Ollama** by default. A **custom endpoint** remains a
...
| D2 | **Default path = browser → F8 → Ollama** for NL-assist/chat. | New `POST /chat`; Studio `generate.ts` default transport switches to the F8 gateway. |
...
## Non-goals (right-sizing — single-process, self-hosted reality)
...
- **No per-instance NL routing preference in v1** — the browser NL routing choice stays a single
...
- `Fallen8ChatOptions` (`Fallen8:Chat:*`) + `Fallen8ChatProvider` — `IsEnabled`, `IsLoaded`, an Ollama
  chat client (`OllamaSharp.IOllamaApiClient.ChatAsync`, already on the current package set — no new
  NuGet). Backend `Ollama` reuses the sidecar endpoint. Disabled ⇒ `POST /chat` → 403.
- `POST /chat` on a new `ChatController` (`[Fallen8Level]` — instance-wide, no `/ns/{ns}` twin;
  ...
  `[RequestSizeLimit(1_048_576)]` (reuse the `/embedding/text` pattern — **no new `MaxInputBytes`
  knob**).
  - **Request:** `{ messages: [{role, content}], options?: {temperature?, …} }` — **no `model`**
...
- **`GET /status` (additive, non-breaking):** add an optional `chat` block mirroring the existing
  ...
  `embedding` block is left exactly as-is** (no "trim to loaded" — that was a mistaken breaking
...
ConfigREST {
  ...
  security: { apiKeyRequired },               // boolean only — never the key value
}
```

- **Redaction:** never emit `Fallen8:Security:ApiKey`. `Otlp.Endpoint` is emitted as configured
  (this codebase's OTLP options carry only an endpoint — no creds/headers to redact).
- **GPU probe (`gpu?`):** best-effort. Read the Ollama endpoint from `IOptions<Fallen8*Options>`
  ...
  unloads idle models), documented as such on the DTO and in the UI — not static config.
...
- The NL store already persists `mode: 'builtin' | 'custom'` (zustand, version 1, with a migrate
  fn). **Rename `builtin` → `instance`** and **bump persist version to 2** with a migration mapping
  stored `'builtin'` → `'instance'` (otherwise those users silently fall through to custom-mode
...
- **Egress notice:** custom-direct keeps the "text leaves this machine" notice. Instance mode shows
  **no** egress notice — the prompt goes to the same instance the user already trusts with their
...
- **This instance (read-only server config)** — sourced from `GET /config` via a new `useConfig`
  hook: the **Semantic providers** view (embedding + chat: backend/model/…/loaded, GPU where
  reported) and an **Observability** status line ("pushing metrics+traces+logs to `<endpoint>`" /
  "Prometheus at `/metrics`" / "off") with a **Configure…** Radix overlay showing values + the exact
  `Fallen8__…` env keys + a "restart to apply" note (D6).
- **Model routing (your browser)** — the NL backend selector (instance vs custom endpoint),
...
| `POST` | `/embedding/text` | unchanged | unchanged | Existing text→vector proxy (embedding half). |
| `GET` | `/statistics` | unchanged | unchanged | Untouched. |

## Config keys (new)

```
Fallen8:Chat:Enabled           (bool, default false — 403 when off)
...
Fallen8:Chat:Ollama:Model      (default phi4-f8-mini — SERVER-owned; no client override)
Fallen8:Chat:TimeoutSeconds    (proxy timeout → 504 on exceed)
```

Compose: `F8_CHAT` (default true when the sidecar is present) → `Fallen8__Chat__Enabled`, reusing the
existing Ollama sidecar + `F8_DELEGATE_REPO` model. GPU wiring unchanged (docker-compose.gpu.yml).
`OLLAMA_ORIGINS` is no longer needed for the *default* path (F8→Ollama is server-to-server); it
...
- `POST /chat` is a sensitive endpoint (API-key gated when a key is set, rate-limited, size
  capped) and **excludes message content from telemetry** — same tag hygiene as embeddings.
...
- **nl-assist-feedback-loop**: its inherited premise ("the intent never reaches a Fallen-8 instance;
  spans exclude filter text") is **retired for the default path** — record consciously. Feedback
...
- **nl-assist-finetune / RETRAIN-LOG.md**: model/prompt/drafted-surface unchanged — transport-only.
  **No retrain entry** (conforms to RETRAIN-LOG conventions; recorded consciously).
- **MCP (engine→REST→MCP rule)**: `POST /chat` and `GET /config` are **conscious deferrals**
...
- **OpenAPI snapshot** (`features/done/web-ui/openapi-v0.1.json`): regenerate via
  `scripts/update-openapi-snapshot.ps1` — `OpenApiDocumentTest.MatchesThePinnedSnapshotInventory`
...
- **Architecture diagrams** (mandatory, precise targets):
  ...
  - `docs/architecture.md`: redirect/scope the `studio -.->|assist calls, direct from browser|
    sidecar` edge (line ~54) — the default assist path now flows studio→rest→sidecar (keep a
    custom-direct note); rewrite the "the browser calls the model backend … directly, so no model
...
- **docs**: `docs/studio.md` — Configuration section + NL reroute, the screen-table row (~line 16)
  ...
  — new default failure modes (F8 403/503 vs browser CORS/404). `docs/running.md` — env table gains
  `F8_CHAT`. `docs/semantic-traversal.md` — chat sibling + config home. `README.md` key-feature line
  (~59) reworded ("assist that runs through your instance by default, or a browser-direct custom
  backend"). `docs/delegates.md` is only a see-also link (no FR-26.11 rule) — light touch, not a
  "retire the rule" target. **No standalone `docs/semantic-gateway.md`** — sections in
...
1. **`GET /config` auth** → **API-key-gated** (like `/statistics`); MCP deferral removes the
   anonymous-bridge conflict. (Resolved, D9.)
2. **`/chat` model selection** → **server-owned, no client knob** (D8). (Resolved.)
3. **Instance-mode model discovery** → Studio shows the server-configured model from `GET /config`;
   **no `/api/tags` model picker in v1** (right-sized). Revisit if a picker is wanted.
4. **Feature packaging** → ship as one feature `instance-config`, phased (server gateway → config
   read → Studio → NL reroute → docs/diagrams/screenshots). (Resolved.)
