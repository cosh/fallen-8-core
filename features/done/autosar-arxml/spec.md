...
> *Later:* both halves of that sentence have moved on. `integration-run-lifecycle` made the input an
> ordered SET of files rather than one, and
> [arxml-can-clusters](../arxml-can-clusters/spec.md) added **CAN**, so the scope is now "at least
> one cluster of a kind the reader reads". This paragraph is left as written because it records what
> v1 shipped.
...
| `readOnly` | `true` |
| `entitySummaryTemplate` | `{kind} {arxml.name}, {arxml.descEn}, {arxml.descDe}, {arxml.unit}` (the semantic payload; why each hole is there is section 9) |
...
- **Interest set:** the elements of section 4 are loaded as subtrees and read; everything
  else (`ADMIN-DATA`, data mappings, timing extensions, the rest of the file that is not
  communication topology) streams past without allocation.

  > *Later, measured:* this said "the ~99% of the file", which is wrong by a wide margin. Elements
  > inside interest-set subtrees are the majority of an extract, not a fraction of a percent, so
  > appreciably less streams past than this claimed. The mechanism is as described; only the
  > proportion was invented. Corrected during
  > [arxml-can-clusters](../arxml-can-clusters/findings.md).
...
Validated offline (2026-08) against a real FlexRay system extract. A prototype of exactly this
parsing design read it in seconds, with **zero unresolved references**, and the resulting graph
loaded and answered impact, degree and path queries correctly. The extract and every value
derived from it stay outside the repository (section 11).
...
| --- | --- |
| ~~CAN clusters~~ **DONE** | The trigger fired: real CAN extracts arrived in a vehicle export. Implemented by [arxml-can-clusters](../arxml-can-clusters/spec.md), and the prediction above held - one entity kind, one relation vocabulary, a six-field protocol table, no new setting |
| Ethernet clusters | Still open, and the prediction above is **measurably wrong** for it: an Ethernet cluster carries **no frame layer at all** (zero elements matching `FRAME` anywhere in the real extracts), so `frame` has no instance and roughly ten new kinds appear (VLANs, network endpoints, socket addresses, static socket connections, SOME/IP service instances, coupling elements, DoIP). It is a second model sharing this one's lower half, not another protocol. It also exceeds the transport a job can carry, which is its first problem rather than its last |
...
| A streaming file read on `ProviderContext` | an extract of a few hundred megabytes. Today the file arrives as one string and the validated size is comfortable. *Later:* still not built, and the trigger has now fired six times over - a real Ethernet extract is a size well past that and a whole segment is a size well past that. It belongs to the Ethernet feature, which cannot be done without it |
