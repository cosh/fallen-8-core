...
- **No ANN index.** Unchanged from vector-index: exact SIMD brute force. The recorded extract is
  12,261 entities, and the revisit trigger stays ~1M vectors or a measured p99 above ~100 ms.
...
1. An ARXML run of the full recorded extract (12,261 entities) with `embedSummaries` on completes
