...
- **[medium] Benchmark screen never says which namespace it operates on, while the app header shows the active namespace switcher**  
  **SUPERSEDED, do not implement the recommended fix below.** The premise ("deliberately Fallen-8-level, always `default`") was itself the defect: the screen wrote into `default` whatever the switcher said. It is now namespace-scoped end to end (`/q/{ns}/benchmarks`, `GET /ns/{ns}/generate`, and the generation response names the namespace it wrote), so hard-coding the word "default" into the copy as prescribed here would re-introduce the bug. The observation that the screen must NAME its namespace was right, and is satisfied by the scoped header, which now carries the instance and the namespace. See features/done/graph-namespaces/README.md.  
