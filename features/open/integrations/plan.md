...
One further change is not a finding but a **decision by the owner, taken after phase 8 shipped**: a credential
may now be supplied as its VALUE in the job (`credentialValues`) and not only by name. The form was the evidence
- the only caller who has a secret in hand and nowhere to put it is a person at one, and the original rule sent
them to the runtime's host to write a file. The rule that mattered survives: the value is leased, redacted,
fingerprinted and dropped exactly as a named one is, and a credential still never arrives as a *setting*. The
spec's section 3 is rewritten around two sources, and phase 8's criterion below is rewritten with it.
