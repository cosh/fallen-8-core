...
- [x] `Fallen8BulkIOOptions` (`Fallen8:BulkIO`: `ImportBatchSize`, `MaxLineBytes`,
  `MaxImportRequestBytes`) bound in `Program.cs`; MIT headers everywhere.
- [x] `Helper/JsonlGraphFormat.cs`: the version-1 line schema — meta/vertex/edge serialisation
...
- [x] `BulkController` + `GET /bulk/export` (`application/x-ndjson`,
...
- [x] `[ProducesResponseType]`/`[Produces]` + XML docs; OpenAPI document renders the endpoint.
- [x] Tests: export of a seeded mixed graph parses line-by-line back into the expected
...
- [x] `POST /bulk/import` (`application/x-ndjson`, async, `PipeReader` line loop): 409 on a
  non-empty graph; optional meta line validated (format/version, counts remembered).
- [x] Batching through the single writer: vertex/edge batches of `ImportBatchSize`, one
...
- [x] Fail-fast line errors as problem+json with `lineNumber`, `reason`,
...
- [x] Limits: `MaxLineBytes` enforcement with line number; the per-request
...
- [x] Concurrent-write-during-export test: writer racing a large export; export completes,
  file is internally consistent, imports cleanly.

## Phase 3 — Docs + gate

- [x] `features/open/bulk-import-export/README.md`: curl examples (export to file, import a
...
- [x] Root `README.md`: a short "Bulk export/import (JSONL)" entry pointing at the feature
  README.
- [x] Full `dotnet test` green; build 0 warnings/0 errors.
- [x] Council review per the repo merge gate; fix findings on the branch;
  `git merge --no-ff` to `main`; move `features/open/bulk-import-export/` →
  `features/done/`.

## Progress

- [x] Phase 0 — format + streaming export + pre-stream validation
- [x] Phase 1 — import happy path (empty-target gate, batching, id remap, round-trip)
- [x] Phase 2 — error contract, limits carve-out, WAL/durability + concurrency tests
- [x] Phase 3 — READMEs, council gate, merge + move to done/
