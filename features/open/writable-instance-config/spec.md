...
and **boot-latched**, so they describe the policy this boot actually ran rather than a written but
unapplied one, which `GET /config` reports with its restart-pending flag (phase 2); bridging per-namespace `loadOnStartup` on `f8_namespace`, closing the dead end
...
| 4.3.4 | **Neither a silent no-op nor a silent override**, at three points: **write time** - `PATCH` answers `409` and writes nothing when any key in the batch is environment- or command-line-declared, naming each refused key, its authority and the exact `Fallen8__…` form (no force flag, no stored-but-shadowed value); **read time** - every descriptor carries `source` (`default`/`appSettings`/`userSecrets`/`environment`/`commandLine`/`host`/`override`), resolved by walking providers in reverse; **boot** - one log line per overridden key the environment outranks, per hand-edited key the catalog refuses, and one for a file that could not be read. `host` is the seventh value the implementation added: an in-process host setting (a test host, an embedding host) that a write CAN outrank, reported separately so exactly two sources, `environment` and `commandLine`, mean a row is locked. |
