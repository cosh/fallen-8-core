...
- **Convention tests** — `fallen-8-unittest/CodeQualityTest.cs` fails the suite on the four
  rules CLAUDE.md's Quality gates section lists (headers, console output, local clock,
  version pins).
...
| `Fallen8.ResolveCachedPlugin<T>` / `TryGetLiveElement<T>` | 2 of 3 resolve-cache copies + all 3 snapshot-resolve copies → 1+1 (the generic Activator overload keeps its distinct shape) |
