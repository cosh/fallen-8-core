...
- [ ] `Fallen8ObservabilityOptions` (`Fallen8:Observability` section) bound in `Program.cs`,
  validated (budget ≥ 1, TopN ≥ 1, sampling ratio in [0,1]); defaults per spec §3.4.
- [ ] `fallen-8-core/Diagnostics/Fallen8Diagnostics.cs` (static `ActivitySource
...
- [ ] Verify by test: engine csproj has no new package reference; two engines in one process
...
- [ ] Transaction pipeline (`TransactionManager.cs`): enqueue timestamp on `WorkItem`
...
- [ ] WAL (`WriteAheadLog.cs`): `fallen8.wal.flush.duration` / `.flush.failures` around
...
- [ ] Checkpoint (`Fallen8.Save` / `Load_internal` + `PersistencyFactory`):
  `fallen8.checkpoint.save|load.duration`, `.save|load.bytes`, `.failures{operation}`.
- [ ] Graph/index gauges: `fallen8.graph.vertices|edges`, `fallen8.index.count`,
  `fallen8.index.entries` (aggregate sum; callbacks read published values only, never walk
  structures).
- [ ] Tests (`MeterListener`): counter/histogram assertions per operation; injected WAL append
...
- [ ] `fallen8.transaction.execute` span with the captured enqueue-time parent context
...
- [ ] `fallen8.checkpoint.save` / `fallen8.checkpoint.load` spans (tags: partitions, bytes,
  replayed entry count).
- [ ] apiApp: `fallen8.codegen.compile` spans around both Roslyn compiles in
...
- [ ] Tests (`ActivityListener`): cross-thread parenting (HTTP-shaped parent → transaction
...
- [ ] OTel packages in the apiApp only (`OpenTelemetry.Extensions.Hosting`,
...
- [ ] `Program.cs`: register OTel **only when** Prometheus or OTLP is enabled; meters
...
- [ ] `GET /metrics` mapped when `Prometheus:Enabled=true`; `[AllowAnonymous]` by default,
  API-key-gated when `Prometheus:RequireApiKey=true` (spec §3.7).
- [ ] `GeneratedCodeCache` hit/miss counters + compile duration/failure metrics
  (`NoSQL.GraphDB.App` meter).
- [ ] Startup posture log line (exporters, `/metrics` auth mode) in the existing security-warning voice.
- [ ] Tests (`WebApplicationFactory`): `/metrics` 404 when disabled; 200 + exposition-format
...
- [ ] `AddHealthChecks` + `MapHealthChecks("/healthz")` (liveness) and `"/readyz"` (readiness),
  both `[AllowAnonymous]`, status-only.
- [ ] `StartupState` (readiness flag) set by `DurabilityLifecycleService.StartAsync` after
  load completes (immediately in volatile mode); document the "ready ≈ live once serving"
  honesty note in the endpoint remarks.
- [ ] Tests: both endpoints 200 anonymous with and without an API key configured; readiness
  reflects the flag.
...
- [ ] `StatisticsController` (versioned route, `[ProducesResponseType]`, XML docs) +
...
- [ ] Budget + uniform-stride sampling per spec §3.5 (`ElementBudget` default 1M, exact below,
  sampled-and-flagged above; sampled distinct counts documented as within-sample).
- [ ] Auth: behind the normal fallback policy (no `[AllowAnonymous]`); rate-limited under the
  existing `SensitiveRateLimitPolicy`.
- [ ] Tests: exact correctness on a known small graph (hand-computed degree stats, label and
...
- [ ] Feature `README.md`: metric reference table, example Prometheus scrape config (including
  the `http_headers` snippet for `RequireApiKey`), example `curl /statistics`, OTLP example.
- [ ] Root `README.md`: short "Observability" section.
- [ ] Overhead check: the `write-path-throughput` benchmark path re-run with metrics off vs on
...
- [ ] Phase 0 — options + diagnostics scaffold (per-engine Meter, static ActivitySource)
- [ ] Phase 1 — engine meters: transactions, WAL, checkpoint, graph/index gauges
- [ ] Phase 2 — spans: transaction (cross-thread parent), checkpoint, codegen, algorithms
- [ ] Phase 3 — exporters + `/metrics` + codegen counters + posture log
- [ ] Phase 4 — `/healthz` + `/readyz` readiness tied to startup load
- [ ] Phase 5 — `/statistics` with budgeted sampling, auth, rate limit
- [ ] Phase 6 — docs, overhead benchmark, council gate, merge + move to done/
