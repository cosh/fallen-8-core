...
| Rate limiting on `/embedding/elements` | Concurrency makes the sensitive-endpoint fixed window easier to trip | 429 already degrades and stops dispatch; the default of 1 means no existing deployment changes behaviour until an operator opts in |
| `docker-compose.nahil.yml` | Where an opt-in belongs, beside the ingestion concurrency it already sets | Add `F8_NAHIL_EMBED_CONCURRENCY`-style knob for the integrations service, defaulted so compose behaviour is a deliberate choice rather than a surprise |
