...
**Still open / not done:** A4 (DelegateTransaction `Durable` contract), B5 is now resolved by C11, A6 + G1 fault-injection tests, C12, E2 (api-error-envelope feature). **Flaky test:** an intermittent failure was observed ~3 times during full runs (Group 4, W1, W3), each passing on immediate re-run and not recurring across 8+ later runs; its identity is unconfirmed (it never failed on a trx-logged run). Worth a dedicated flaky-test hunt.
